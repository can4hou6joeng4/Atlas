...
struct NotchIslandSettingsDetailPane: View {
    ...
    private func islandDisplaySettings(prefs: Preferences) -> some View {
        SettingGroup(
            ...
            caption: NotchIslandLocalization.atollText("Atlas owns placement, sizing, and the global shortcut.", locale: locale)
        ) {
            VStack(spacing: 0) {
    ...
    private func moduleAvailabilitySettings(_ module: NotchIslandModule, prefs: Preferences) -> some View {
        ...
        return SettingGroup(
            ...
            caption: NotchIslandLocalization.atollText("Module availability is stored in Atlas; Atoll-specific settings below tune behavior inside the module.", locale: locale)
        ) {
            VStack(spacing: 0) {
