...
// ConfigurationsView keep resolving them within the Atlas module.
...
struct CodexProfileEditorPanel: View {
    ...
    let reactivateAtlas: @MainActor () -> Void
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if vm.draftProviderID == nil {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("应用混合中转时，Atlas 会把该 Key 写入 Codex config.toml 的 experimental_bearer_token，供 Codex 运行时读取。")
                        .font(.sora(10))
                        .foregroundStyle(Color.stxMuted)
                        .fixedSize(horizontal: false, vertical: true)
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(vm.isLoading ? "正在检查本地 CLI 环境" : "本地 CLI 环境将自动检查")
                    .font(.sora(13, weight: .semibold))
                Text("Atlas 会在进入配置页时读取 shell 配置文件并运行 CLI 版本检查；需要 macOS 授权时会直接弹出系统窗口。")
                    .font(.sora(11))
                    .foregroundStyle(Color.stxMuted)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(vm.isLoading ? "正在扫描环境变量冲突" : "环境变量冲突将自动扫描")
                    .font(.sora(13, weight: .semibold))
                Text("Atlas 会检查进程和 shell 配置中的 ANTHROPIC / OPENAI 覆盖变量。")
                    .font(.sora(11))
                    .foregroundStyle(Color.stxMuted)
                    .lineLimit(2)
            }
