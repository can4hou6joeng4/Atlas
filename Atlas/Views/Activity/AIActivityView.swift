    ...
    private var exportPermissionNote: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("SCREEN TIME ACCESS NOT GRANTED")
                ...
                .tracking(1.2)
                .foregroundStyle(.primary)
            Text("No coding-surface activity data to show — Atlas needs Full Disk Access to read macOS Screen Time.")
                .font(.sora(10))
    ...
    private var permissionGate: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("FULL DISK ACCESS REQUIRED")
                ...
                .tracking(1.2)
                .foregroundStyle(.primary)
            Text("Atlas reads macOS Screen Time (the Knowledge database) to see when your coding surfaces and CLI hosts were focused. macOS keeps that file behind Full Disk Access — grant it, then come back to this tab.")
                .font(.sora(10))
                .foregroundStyle(Color.stxMuted)
                .fixedSize(horizontal: false, vertical: true)
            HStack(spacing: 8) {
