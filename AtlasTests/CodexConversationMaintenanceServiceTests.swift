...
struct CodexConversationMaintenanceServiceTests {
    ...
    func previewsAndSyncsProviders() async throws {
        ...
        let snapshot = try await service.providerSyncSnapshot(targetProvider: "Atlas")
        ...
        let result = try await service.runProviderSync(targetProvider: "Atlas")
        ...
        #expect(updatedRollout.contains(#""model_provider":"Atlas""#))
        ...
        #expect(statement.columnString(0) == "Atlas")
    ...
    func providerSyncLockPreventsConcurrentRewrites() async throws {
        ...
        do {
            _ = try await CodexConversationMaintenanceService(paths: paths).runProviderSync(targetProvider: "Atlas")
    ...
    func providerSyncRollsBackRolloutsWhenSQLiteFails() async throws {
        ...
        #expect(rolledBack.contains(#""model_provider":"Atlas""#) == false)
    }
    ...
    func recycleBinListRestoreDelete() async throws {
        ...
        try Self.writeBackup(
            ...
            thread: ThreadFixture(id: "t1", title: "Deleted Thread", provider: "Atlas", cwd: "/tmp/project", rolloutPath: rollout.path),
            fileContent: Self.rollout(id: "t1", provider: "Atlas", cwd: "/tmp/project")
        )
        ...
        #expect(statement.columnString(1) == "Atlas")

        try Self.writeBackup(
            to: backupURL,
            dbPath: paths.stateSQLiteURL.path,
            thread: ThreadFixture(id: "t2", title: "Trash", provider: "Atlas", cwd: "/tmp/project", rolloutPath: ""),
            fileContent: nil
        )
    ...
    func recycleBinInvalidStates() async throws {
        ...
        try Self.writeBackup(
            ...
            thread: ThreadFixture(id: "foreign", title: "Foreign", provider: "Atlas", cwd: "/tmp/project", rolloutPath: ""),
            fileContent: nil
        )
    ...
    func recycleRestoreReportsFileConflict() async throws {
        ...
        try Self.writeBackup(
            ...
            dbPath: paths.stateSQLiteURL.path,
            thread: ThreadFixture(id: "conflict", title: "Conflict", provider: "Atlas", cwd: "/tmp/project", rolloutPath: rollout.path),
            fileContent: Self.rollout(id: "conflict", provider: "Atlas", cwd: "/tmp/project")
        )
