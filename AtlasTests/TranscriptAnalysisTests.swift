...
struct TechnicalTermDictionaryTests {
    ...
    func repositoryMergePrecedenceAndDigest() async throws {
        ...
        let homeRoot = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".atlas-tests", isDirectory: true)
        ...
        let parentTerms = projectRoot.appendingPathComponent(".atlas/terms.json")
        let childTerms = childRoot.appendingPathComponent(".atlas/terms.json")
    ...
    func dictionaryStoreFiltersByScopeCategoryAndQuery() async throws {
        ...
        let childRoot = projectRoot.appendingPathComponent("child", isDirectory: true)
        let projectTerms = childRoot.appendingPathComponent(".atlas/terms.json")
    ...
    private static func bundledResourceURL(name: String, extension ext: String) throws -> URL {
        ...
        let fallback = repoURL
            .appendingPathComponent("Atlas/Resources/TranscriptAnalysis", isDirectory: true)
...
struct TranscriptTermExtractorTests {
    ...
    func extractsMixedTranscriptTerms() async throws {
        ...
        let messages = [
            Self.message(
                role: .user,
                text: "请分析 SessionStore 和 SwiftUI。运行 `bash scripts/run-debug.sh` 后遇到 build failed，路径 Atlas/Views/Sessions/SessionDetailView.swift"
            ),
            Self.message(
                role: .assistant,
    ...
    func skipsNaturalLanguageForCodeHeavyText() async throws {
        ...
        let jsonLine = #"{"type":"tool_result","payload":{"path":"Atlas/Services/SessionStore.swift","status":"ok","id":"abc-123"}}"#
        let messages = [
    ...
    private static func session(id: String, provider: ProviderKind) -> Session {
        Session(
            ...
            externalID: "analysis",
            provider: provider,
            projectDirectoryName: "-Users-dev-Atlas",
