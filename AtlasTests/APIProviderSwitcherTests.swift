...
struct APIProviderSwitcherTests {
    ...
    func codexProviderPreservesCommonConfig() async throws {
        ...
        #expect(config.contains(#"model_provider = "Atlas""#))
        #expect(config.contains("[model_providers.Atlas]"))
    ...
    @Test("Codex official channel clears only Atlas managed provider")
    func codexOfficialChannelClearsOnlyAtlasProvider() async throws {
        let temp = try TempDir.make()
        defer { try? FileManager.default.removeItem(at: temp) }
        ...
        let config = try String(contentsOf: codexHome.appendingPathComponent("config.toml"), encoding: .utf8)
        #expect(auth == authJSON)
        #expect(config.contains(#"model_provider = "Atlas""#) == false)
        #expect(config.contains("[model_providers.Atlas]") == false)
    ...
    @Test("Codex channel status uses selected Atlas profile name")
    func codexChannelStatusUsesSelectedAtlasProfileName() async throws {
        let temp = try TempDir.make()
        defer { try? FileManager.default.removeItem(at: temp) }
