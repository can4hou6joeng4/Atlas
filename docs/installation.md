...
Atlas publishes source code, GitHub Releases, and the Sparkle appcast from
...
2. Download `Atlas-<version>.dmg` or `Atlas-<version>.zip`.
3. If you downloaded the DMG, open it and drag `Atlas.app` onto the
   `Applications` shortcut shown in the installer window.
4. If you downloaded the zip fallback, unzip it and move `Atlas.app` into
   `/Applications`.
5. Launch Atlas.
...
Atlas embeds Sparkle 2. Settings -> About includes **Check for Updates...**,
and scheduled update checks are enabled by default.

Automatic updates require:

- `SUPublicEDKey` in `Atlas/App/Info.plist`.
